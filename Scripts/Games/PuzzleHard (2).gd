...
func _play_pieces_final_animation() -> void:
	var cadenas := get_node("Cadenas")
	if cadenas == null:
		return

	var first_piece: Node = null
	for child in cadenas.get_children():
		if child.name.begins_with("Pieza"):
			first_piece = child
			break
	...
	# piezaPuzzle.gd ya se encarga de recorrer todos los slots/piezas
	# y animarlas en secuencia la primera vez que se llama.
	await first_piece._animacion_finalizado()
...
func victory():
	...
	await _play_pieces_final_animation()
...
func rondaWin():
	...
	# Una sola llamada que anima todas las piezas en secuencia
	await _play_pieces_final_animation()
