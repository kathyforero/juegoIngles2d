...
@export var palabra = "TRAIN"
@export var palabraES = "Tren"
...
var _classic_transitioning: bool = false # lock para Classic/Practice
...
func _process(_delta):
	...
	# Lock anti "doble transición" (coroutines duplicadas)
	...
	# --- MODO CLÁSICO ---
	if all_correct:
		# Fin del juego (última ronda)
		if rondaActual >= rondas and !gano:
			_classic_transitioning = true
			gano = true
			await _await_letters_settle()  # <- clave: no iniciar "Gana" antes de la última animación
			await victory()
			_classic_transitioning = false
			return
...
func _cargar_palabras_desde_banco_medium():
	palabras.clear()
	...
	for ejercicio in BancoOrderIt.medium:
