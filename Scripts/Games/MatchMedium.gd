...
var velocidad = 20
var valorNivel = 100
var tiempoCronometro = 110  # Tiempo en segundos del cronómetro.
...
func _ready():
	...
	# Limpiar listas (por si reentras a la escena)
	images.clear()
	names.clear()
	...
	# 1) Poner el tiempo en el HUD
	$Box_inside_game.time_seconds = tiempoCronometro
	
	# 2) Inicializar HUD y textos
	if not _is_practice:
	...
	# 3) Instanciar escenas necesarias para el juego.
...
func load_medium_mode_animals():
	animals.clear()
	if BancoMatchIt.medium.is_empty():
		push_error("BancoMatchIt.medium is empty. Check Banco_MatchIt.gd / Banco_MatchIt.json.")
		return
	animals = BancoMatchIt.medium.duplicate(true)
...
func assign_images_and_names():
	...
	# Pick 15 unique animals for this game
	for i in range(15):
		var selected_key = keys.pop_front()
...
func iniciar_juego():
	...
	# TIME ATTACK: ronda infinita de 3 items
	if _is_time_attack:
		emit_signal("update_level", str(Score.levels_completed))
...
func cargar_ronda():
	...
	# TIME ATTACK: recargar 3 nuevos siempre
	if _is_time_attack:
		if _ta_finished:
			return
		_preparar_3_animales_para_ronda_time_attack()
...
# Asigna imágenes/textos con el mismo método (shuffle de pares e independiente del texto)
...
# TIME ATTACK: preparar 3 animales por ronda, y si se acaban, recargar banco (permite repetir)
...
func finish_time_attack() -> void:
	...
	# Mostrar overlay de victoria (tu mismo estilo)
...
func _actualizar_puntajes(path):
	...
	var diff_key := "medium"  # Este script es SOLO para Medium
	...
	if FileAccess.file_exists(path):
		...
		# Asegurar estructura y campos nuevos
