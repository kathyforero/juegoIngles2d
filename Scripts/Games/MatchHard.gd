...
func load_hard_mode_animals():
	animals.clear()
	if BancoMatchIt.hard.is_empty():
		push_error("BancoMatchIt.hard is empty. Check Banco_MatchIt.gd / Banco_MatchIt.json.")
		return
	animals = BancoMatchIt.hard.duplicate(true)
	...
	if FileAccess.file_exists(path):
		...
		for diff in ["easy", "medium", "hard"]:
			if not puntajes.has(diff):
				puntajes[diff] = {"velocidad": 0,"precision": 0,"niveles": 0,"best_score": 0,"name": ""}
			else:
		...
		if totalActual >= best_prev:
			...
			puntajes[diff_key] = {"velocidad": velocidad,"precision": precisionActual,"niveles": valorNivel,"best_score": totalActual,"name": nombre_guardado}
		...
		var content = {
			...
			"hard": {"velocidad": velocidad,"precision": precisionActual,"niveles": valorNivel,"best_score": totalActual,"name": ""}
		}
...
func _actualizar_puntajes_time_attack(path:String, totalActual:int) -> void:
	var is_new_record := false
	var diff_key := "hard" # este script es HARD
	...
	for diff in ["easy", "medium", "hard"]:
		if not puntajes["time_attack"].has(diff):
			puntajes["time_attack"][diff] = {"best_score": 0,"name": "---","levels": 0,"seconds": 0,"precision": 0}
...
func crear_flecha(imagen_box: Node2D, texto_box: Node2D) -> void:
	...
	line.z_index = 10
	add_child(line)
	arrows[imagen_box.value] = {"line": line,"imagen": imagen_box,"texto": texto_box}
